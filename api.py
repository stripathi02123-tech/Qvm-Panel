#!/usr/bin/env python3
"""
QVM Panel - Comprehensive REST API
Full management API for QVM Panel
"""
...
def api_info():
    ...
    return jsonify({
        ...
        'api': {
            'name': 'QVM Panel API',
            'version': 'v1',
            'description': 'Comprehensive REST API for QVM Panel management',
...
def api_get_vps(vps_id):
    """Get VPS details"""
    from avm import get_db, get_vps_by_id, run_sync, get_container_stats
...
def api_create_vps():
    """Create new VPS"""
    from avm import get_db, run_sync, execute_lxc, apply_lxc_config, configure_container_ip, apply_internal_permissions
    ...
    try:
        ...
        # This would call the actual VPS creation functions from avm.py
        
        return jsonify({
            'success': True,
            'message': 'VPS creation started',
            'note': 'Full implementation requires integration with avm.py VPS creation logic'
        }), 201
    except Exception as e:
...
def api_start_vps(vps_id):
    """Start VPS"""
    from avm import get_vps_by_id, run_sync, execute_lxc
...
def api_delete_vps(vps_id):
    """Delete VPS"""
    from avm import get_db, get_vps_by_id, run_sync, execute_lxc
...
def api_get_node(node_id):
    """Get node details"""
    from avm import get_node, run_sync, get_host_stats
...
def api_system_info():
    ...
    from avm import get_host_cpu_usage, get_host_ram_usage, get_host_disk_usage, get_host_uptime
    import platform
...
def api_resize_vps(vps_id):
    ...
    from avm import get_db, get_vps_by_id
...
def api_bulk_start_vps():
    """Start multiple VPS"""
    from avm import get_db, run_sync, execute_lxc
