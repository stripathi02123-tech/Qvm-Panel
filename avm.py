#!/usr/bin/env python3
# avm.py - QVM Panel - Full Web-Based LXC Container VPS Management System
# Version: 1.0
...
PANEL_NAME = os.getenv('PANEL_NAME', 'QVM Panel')
PANEL_VERSION = os.getenv('PANEL_VERSION', 'v1.0')
PANEL_DEVELOPER = os.getenv('PANEL_DEVELOPER', 'QVM Panel')
...
try:
    _log_file = os.path.join(tempfile.gettempdir(), 'avm.log')
...
from db import (
    get_database, init_db as mongo_init_db, get_setting as mongo_get_setting,
    set_setting as mongo_set_setting, log_activity as mongo_log_activity,
    create_notification as mongo_create_notification,
    get_user_notifications as mongo_get_user_notifications,
    mark_notification_read as mongo_mark_notification_read,
    mark_all_notifications_read as mongo_mark_all_notifications_read,
    get_unread_notifications_count as mongo_get_unread_notifications_count,
    generate_api_key as mongo_generate_api_key, Document as MongoDocument
)
from bson import ObjectId


def generate_api_key(length=64):
    return mongo_generate_api_key(length)


def get_setting(key: str, default: Any = None) -> Any:
    return mongo_get_setting(key, default)


def set_setting(key: str, value: str):
    mongo_set_setting(key, value)


def log_activity(user_id, action, resource_type=None, resource_id=None, details=None):
    mongo_log_activity(user_id, action, resource_type, resource_id, details)


def create_notification(user_id, ntype, title, message, data=None, expires_in=None):
    mongo_create_notification(user_id, ntype, title, message, data, expires_in)


def get_user_notifications(user_id, unread_only=False, limit=50):
    return mongo_get_user_notifications(user_id, unread_only, limit)


def mark_notification_read(notification_id, user_id):
    return mongo_mark_notification_read(notification_id, user_id)


def mark_all_notifications_read(user_id):
    return mongo_mark_all_notifications_read(user_id)


def get_unread_notifications_count(user_id):
    return mongo_get_unread_notifications_count(user_id)


class RowProxy:
    """Dict wrapper that supports index access like sqlite3.Row: row[0], row['key']"""
    def __init__(self, d):
        self._dict = d
    ...
    def __getattr__(self, name):
        ...
        try:
            ...
            raise AttributeError(f"'RowProxy' object has no attribute '{name}'")


class MongoDBCursor:
    """SQLite-like cursor for MongoDB with full SQL compatibility"""
    ...
    def execute(self, query, params=None):
        ...
        try:
            if q_upper.startswith('SELECT'):
                ...
                self._do_delete(query, params)
            elif q_upper.startswith('CREATE') or q_upper.startswith('ALTER'):
                pass
        except Exception as e:
            logger.error(f"Cursor execute error: {e} | query: {query[:80]}")
            self._result = []
            self.rowcount = 0

    def _parse_table(self, query):
        q = query.upper()
        for kw in ['DELETE FROM', 'FROM', 'INTO', 'UPDATE']:
            idx = q.find(kw)
            if idx != -1:
    ...
    def _parse_insert_columns(self, query):
        """Parse column names from INSERT INTO table (col1, col2, ...) VALUES (...)"""
        q = query.upper()
        into_idx = q.find('INTO')
        if into_idx == -1:
            return None
        rest = query[into_idx+4:].strip()
        # Find opening paren for column list
        paren_idx = rest.find('(')
        if paren_idx == -1:
            return None
        close_idx = rest.find(')', paren_idx)
        if close_idx == -1:
            return None
        cols_str = rest[paren_idx+1:close_idx]
        cols = [c.strip().strip('`"\'') for c in cols_str.split(',')]
        return cols

    def _parse_where(self, query, params, start_pi=0):
        doc = {}
        q = query.upper()
        idx = q.find('WHERE')
        if idx == -1:
            return doc, start_pi
        clause = query[idx+5:].strip()
        for kw in ['ORDER BY', 'LIMIT', 'GROUP BY']:
            i = clause.upper().find(kw)
            if i != -1:
                clause = clause[:i].strip()
        parts = [p.strip() for p in clause.split(' AND ') if p.strip()]
        pi = start_pi
        for p in parts:
            # Handle OR prefix
            if p.upper().startswith('OR '):
                p = p[3:].strip()
            if '=' in p and '!=' not in p and '<>' not in p:
                ...
                if v == '?':
                    if pi < len(params):
                        doc[k] = {'$lt': params[pi]}; pi += 1
            elif ' LIKE ' in p.upper():
                parts_l = p.upper().split('LIKE', 1)
                k = parts_l[0].strip().strip('`"\'')
                v = parts_l[1].strip().strip('`"\'')
                if v == '?':
                    if pi < len(params):
                        doc[k] = {'$regex': params[pi].replace('%','.*').replace('_','.'), '$options': 'i'}
                        pi += 1
            elif 'IN (' in p.upper():
                in_idx = p.upper().find('IN (')
                k = p[:in_idx].strip().strip('`"\'')
                vals_str = p[in_idx+4:p.rfind(')')].strip()
                if vals_str == '?':
                    if pi < len(params):
                        val = params[pi]; pi += 1
    ...
    def _fix_id_filter(self, flt):
        """Convert 'id' field to '_id' for MongoDB compatibility"""
        if 'id' in flt:
            val = flt.pop('id')
            from bson import ObjectId
            try:
                flt['_id'] = ObjectId(val)
            except Exception:
                flt['_id'] = val
        return flt

    def _do_select(self, query, params):
        table = self._parse_table(query)
        coll = self._db[table]
        flt, _ = self._parse_where(query, params)
        flt = self._fix_id_filter(flt)
        q_up = query.upper()

        # Handle aggregates: COUNT, SUM, AVG, MAX, MIN
        if 'COUNT(' in q_up or 'SUM(' in q_up or 'AVG(' in q_up or 'MAX(' in q_up or 'MIN(' in q_up:
            self._do_aggregate(table, coll, flt, query, q_up)
            return

        limit = None
        li = q_up.find('LIMIT')
        if li != -1:
            try: limit = int(query[li+6:].strip().split()[0])
            except: pass
        cur = coll.find(flt)
        if limit:
            cur = cur.limit(limit)
        self._result = list(cur)
        self.rowcount = len(self._result)

    def _do_aggregate(self, table, coll, flt, query, q_up):
        """Handle COUNT, SUM, AVG, MAX, MIN aggregates using MongoDB aggregation pipeline"""
        pipeline = []
        if flt:
        ...
        if 'COUNT(' in q_up:
            ...
            self._result = [{'_result': count}]
        elif 'SUM(' in q_up:
            field = self._extract_agg_field(query, 'SUM')
            if field:
                pipeline.append({'$group': {'_id': None, 'result': {'$sum': f'${field}'}}})
                result = list(coll.aggregate(pipeline))
                val = result[0]['result'] if result else 0
                self._result = [{'_result': val}]
            else:
                self._result = [{'_result': 0}]
        elif 'AVG(' in q_up:
            field = self._extract_agg_field(query, 'AVG')
            if field:
            ...
            field = self._extract_agg_field(query, 'MAX')
            if field:
                pipeline.append({'$group': {'_id': None, 'result': {'$max': f'${field}'}}})
                result = list(coll.aggregate(pipeline))
                val = result[0]['result'] if result else None
                self._result = [{'_result': val}]
            else:
                self._result = [{'_result': None}]
        elif 'MIN(' in q_up:
            field = self._extract_agg_field(query, 'MIN')
            if field:
    ...
    def _extract_agg_field(self, query, func):
        """Extract field name from COUNT(field), SUM(field), etc."""
        q = query.upper()
        idx = q.find(f'{func}(')
        if idx == -1:
            return None
        ...
        if end == -1:
            return None
        field = query[start:end].strip()
        if field == '*' or field == '1':
            return None
        return field.strip('`"\'')

    def _do_insert(self, query, params):
        table = self._parse_table(query)
        coll = self._db[table]
        cols = self._parse_insert_columns(query)
        if params and cols and len(cols) == len(params):
            doc = dict(zip(cols, params))
        elif isinstance(params, dict):
            doc = dict(params)
        elif params and len(params) == 1 and isinstance(params[0], dict):
            doc = dict(params[0])
        else:
            doc = {}
        try:
            r = coll.insert_one(doc)
            self.lastrowid = str(r.inserted_id)
            self.rowcount = 1
        except Exception as e:
            logger.error(f"Insert error: {e}")
            self.rowcount = 0

    def _do_update(self, query, params):
        table = self._parse_table(query)
        coll = self._db[table]
        q_up = query.upper()
        si = q_up.find('SET')
        ...
        update_doc = {}
        if si != -1:
            set_clause = query[si+3:wi].strip() if wi != -1 else query[si+3:].strip()
            for part in set_clause.split(','):
                ...
                if '=' in part:
                    ...
                    if v == '?':
                        if pi < len(params):
                            update_doc[k] = params[pi]; pi += 1
                    elif v.upper() == 'NULL':
                        update_doc[k] = None
                    elif v.upper() == 'CURRENT_TIMESTAMP':
                        update_doc[k] = datetime.now().isoformat()
                    elif v.startswith("'") and v.endswith("'"):
                        update_doc[k] = v[1:-1]
                    else:
                        try: update_doc[k] = int(v)
                        except:
                            try: update_doc[k] = float(v)
                            except: update_doc[k] = v
        flt, _ = self._parse_where(query, params, start_pi=pi)
        flt = self._fix_id_filter(flt)
        if update_doc:
            r = coll.update_many(flt, {'$set': update_doc})
            self.rowcount = r.modified_count
        elif flt:
            # UPDATE without SET but with WHERE - no-op
            self.rowcount = 0

    def _do_delete(self, query, params):
        table = self._parse_table(query)
        coll = self._db[table]
        flt, _ = self._parse_where(query, params)
        flt = self._fix_id_filter(flt)
        r = coll.delete_many(flt)
        self.rowcount = r.deleted_count

    def _doc_to_row(self, doc):
        """Convert MongoDB document to sqlite3.Row-like dict with 'id' from '_id'"""
        row = dict(doc)
        oid = doc.get('_id')
        if oid is not None and '_id' in row:
            del row['_id']
            row['id'] = str(oid)
        elif '_id' in row:
            del row['_id']
        # Ensure all JSON fields are strings for compatibility
        for k in ['tags', 'ip_addresses', 'ip_aliases']:
    ...
    def fetchall(self):
        ...
        self._result = None
        return [self._doc_to_row(doc) for doc in result]


class MongoDBWrapper:
    """Wraps MongoDB database to provide sqlite3-like cursor interface"""
    def __init__(self, db):
        self._db = db

    def cursor(self):
        return MongoDBCursor(self._db)
...
@contextmanager
def get_db():
    """MongoDB-compatible context manager"""
    db = get_database()
    wrapper = MongoDBWrapper(db)
    try:
        yield wrapper
    except Exception as e:
...
def check_maintenance():
    if request.endpoint == 'static' or request.path.startswith('/static/'):
        return None
    if request.endpoint in ['login', 'logout', 'register', 'health', 'favicon', 'serve_static']:
        return None
    ...
    if maintenance_mode:
        ...
        return render_template('maintenance.html',
                             ...
                             panel_name=get_setting('site_name', 'QVM Panel')), 503
...
class User(UserMixin):
    def __init__(self, user_data):
        ...
        self.google_username = user_data.get('google_username')
        self.google_email = user_data.get('google_email')
        self.google_avatar = user_data.get('google_avatar')
...
if socketio:
    ...
    @socketio.on('join_deploy_room')
    def handle_join_deploy_room():
        if current_user.is_authenticated:
            join_room(f'deploy_{current_user.id}')
...
def create_vps(user_id: int, node_id: int, container_name: str, ram: str, cpu: str, storage: str,
               ...
               virt_kvm: bool = False, virt_docker: bool = False,
               root_password: Optional[str] = None) -> int:
    ...
    with get_db() as conn:
        ...
        cur.execute('''INSERT INTO vps
            (user_id, node_id, container_name, hostname, ram, cpu, storage, config, os_version,
             ...
             expires_at, expiration_days, auto_suspend_enabled, renewal_count, auto_backup_enabled,
             virt_kvm, virt_docker)
            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)''',
            (user_id, node_id, container_name, hostname or container_name, ram, cpu, storage, config, os_version,
             ...
             1 if auto_backup_enabled else 0,
             1 if virt_kvm else 0, 1 if virt_docker else 0, root_password))
        conn.commit()
        ...
        log_activity(user_id, 'create_vps', 'vps', str(vps_id), {'container': container_name, 'kvm': virt_kvm, 'docker': virt_docker})
        ...
        if auto_suspend_enabled and expiration_days > 0:
            create_notification(user_id, 'success', 'VPS Created', 
                              f'Your VPS {container_name}{feature_str} has been created successfully. It will auto-suspend in {expiration_days} days.')
        else:
            create_notification(user_id, 'success', 'VPS Created', f'Your VPS {container_name}{feature_str} has been created successfully.')
...
def index():
    if current_user.is_authenticated:
        return redirect(url_for('dashboard'))
    return render_template('index.html', panel_name=get_setting('site_name', 'QVM Panel'))
...
def login():
    ...
    return render_template('login.html', panel_name=get_setting('site_name', 'QVM Panel'))
...
def two_factor():
    ...
    return render_template('2fa.html', panel_name=get_setting('site_name', 'QVM Panel'))
...
def register():
    ...
    if request.method == 'POST':
        ...
        if not terms:
            ...
            return render_template('register.html', panel_name=get_setting('site_name', 'QVM Panel'))
        ...
        create_notification(user_id, 'success', 'Welcome!', f'Welcome to {get_setting("site_name", "QVM Panel")}! Your account has been created.')
...
def discord_auto_register(discord_id, discord_username, discord_email, discord_avatar):
    ...
    try:
        with get_db() as conn:
            ...
            create_notification(user.id, 'success', 'Welcome!', f'Welcome to {get_setting("site_name", "QVM Panel")}, {username}! Your account has been created.')
            flash(f'Welcome to {get_setting("site_name", "QVM Panel")}, {username}! Your account has been created.', 'success')
            return redirect(url_for('dashboard'))
...
def google_login():
    ...
    if not get_setting('google_auth_enabled', '0') == '1':
        flash('Google authentication is not enabled', 'danger')
        return redirect(url_for('login'))
    ...
    session['google_oauth_action'] = 'login'
...
def google_register():
    ...
    state = secrets.token_urlsafe(32)
    session['google_oauth_state'] = state
    session['google_oauth_action'] = 'register'
...
def google_callback():
    ...
    try:
        ...
        logger.error(f"Google user fetch error: {e}")
        flash('Failed to fetch user information from Google', 'danger')
        return redirect(url_for('login'))
    
    google_id = google_user['id']
    google_username = google_user.get('name', google_user.get('email', '').split('@')[0])
    google_email = google_user.get('email')
    google_avatar = google_user.get('picture')
    
    action = session.get('google_oauth_action', 'login')
    session.pop('google_oauth_state', None)
    session.pop('google_oauth_action', None)
    
    with get_db() as conn:
        cur = conn.cursor()
        cur.execute('SELECT * FROM users WHERE google_id = ?', (str(google_id),))
        existing_user = cur.fetchone()
        ...
        if action == 'login':
            if existing_user:
                ...
                login_user(user, remember=True)
                cur.execute('UPDATE users SET last_login = ?, last_active = ?, google_username = ?, google_avatar = ?, google_email = ? WHERE id = ?',
                          (datetime.now().isoformat(), datetime.now().isoformat(), google_username, google_avatar, google_email, user.id))
                conn.commit()
                log_activity(user.id, 'login_google', 'auth', str(user.id))
                flash(f'Welcome back, {user.username}!', 'success')
                return redirect(url_for('dashboard'))
            else:
                if get_setting('google_auto_register', '0') == '1':
                    return google_auto_register(google_id, google_username, google_email, google_avatar)
                else:
                    flash('No account found with this Google account. Please register first.', 'warning')
                    return redirect(url_for('register'))
        
        elif action == 'register':
            if existing_user:
                flash('This Google account is already registered. Please log in instead.', 'warning')
                return redirect(url_for('login'))
            else:
                return google_auto_register(google_id, google_username, google_email, google_avatar)
...
def google_auto_register(google_id, google_username, google_email, google_avatar):
    """Auto-register a new user from Google"""
    try:
        with get_db() as conn:
            cur = conn.cursor()
            
            base_username = re.sub(r'[^a-z0-9_]', '', google_username.lower())
            ...
            email = google_email if google_email else f"{username}@google.local"
            counter = 1
            while True:
                cur.execute('SELECT id FROM users WHERE email = ?', (email,))
                if not cur.fetchone():
                    break
                email = f"{username}{counter}@google.local"
                counter += 1
            ...
            cur.execute('''INSERT INTO users 
                (username, email, password_hash, is_admin, is_main_admin, created_at, last_login, last_active, api_key, preferences,
                 google_id, google_username, google_email, google_avatar, profile_picture)
                VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)''',
                (username, email, password_hash, 0, 0, now, now, now, api_key, '{}',
                 str(google_id), google_username, google_email, google_avatar, google_avatar))
            ...
            log_activity(user.id, 'register_google', 'auth', str(user.id))
            create_notification(user.id, 'success', 'Welcome!', f'Welcome to {get_setting("site_name", "QVM Panel")}, {username}!')
            flash(f'Welcome to {get_setting("site_name", "QVM Panel")}, {username}!', 'success')
            return redirect(url_for('dashboard'))
    except Exception as e:
        logger.error(f"Google auto-register error: {e}")
        flash('Failed to create account. Please try again.', 'danger')
        return redirect(url_for('register'))
...
def google_unlink():
    ...
    try:
        with get_db() as conn:
            cur = conn.cursor()
            cur.execute('''UPDATE users 
                SET google_id = NULL, google_username = NULL, google_avatar = NULL, google_email = NULL
                WHERE id = ?''',
                (current_user.id,))
            conn.commit()
        flash('Google account unlinked successfully', 'success')
        log_activity(current_user.id, 'unlink_google', 'auth', str(current_user.id))
    except Exception as e:
        logger.error(f"Google unlink error: {e}")
        flash('Failed to unlink Google account', 'danger')
    return redirect(url_for('profile'))
...
def forgot_password():
    ...
    return render_template('forgot_password.html', panel_name=get_setting('site_name', 'QVM Panel'))
...
def reset_password(token):
    ...
    return render_template('reset_password.html', token=token, panel_name=get_setting('site_name', 'QVM Panel'))
...
def notifications():
    ...
    return render_template('notifications.html',
                          panel_name=get_setting('site_name', 'QVM Panel'),
...
@app.route('/vps/<vps_id>/files/browse', methods=['POST'])
@login_required
...
@app.route('/vps/<vps_id>/files/download', methods=['POST'])
@login_required  
...
@app.route('/vps/<vps_id>/files/rename', methods=['POST'])
@login_required
...
# VPS Control Routes
# ============================================================================
@app.route('/vps/<vps_id>/control/<action>', methods=['POST'])
@login_required
...
@app.route('/vps/<vps_id>/ssh', methods=['POST'])
@login_required
def vps_ssh(vps_id):
    vps = get_vps_by_id(vps_id)
    if not vps or (vps['user_id'] != current_user.id and not current_user.is_admin):
    ...
    try:
        ...
        node_id = vps['node_id']
        session_name = f"avm-session-{datetime.now().strftime('%Y%m%d%H%M%S')}"
...
@app.route('/vps/<vps_id>/reinstall', methods=['POST'])
@login_required
...
@app.route('/vps/<vps_id>/rename', methods=['POST'])
@login_required
...
@app.route('/vps/<vps_id>/notes', methods=['POST'])
@login_required
...
@app.route('/vps/<vps_id>/suspended')
@login_required
...
@app.route('/vps/<vps_id>/expiration')
@login_required
...
def admin_users():
    ...
    cursor_users = db.users.find(user_filter).sort('_id', 1).skip(offset).limit(per_page)
    users = []
    for u in cursor_users:
        uid = str(u['_id'])
        u['id'] = uid
        u['vps_count'] = db.vps.count_documents({'user_id': uid})
        pa = db.port_allocations.find_one({'user_id': uid})
        u['port_quota'] = pa.get('allocated_ports', 10) if pa else 10
        u['used_ports'] = pa.get('used_ports', 0) if pa else 0
        users.append(u)
...
def admin_user_detail(user_id):
    ...
    # Get VPS list for this user
    vps_cursor = db.vps.find({'user_id': user_id}).sort('created_at', -1)
    vps_list = []
    for v in vps_cursor:
        v['id'] = str(v['_id'])
        try:
            v['shared_with'] = json.loads(v['shared_with']) if v.get('shared_with') else []
        except:
            v['shared_with'] = []
        vps_list.append(v)
    pa = db.port_allocations.find_one({'user_id': user_id})
    allocated_ports = pa.get('allocated_ports', 0) if pa else 0
    used_ports = pa.get('used_ports', 0) if pa else 0
    forwards = list(db.port_forwards.find({'user_id': user_id}).sort('created_at', -1))
    activities = list(db.activity_logs.find({'user_id': user_id}).sort('created_at', -1).limit(50))
...
@app.route('/admin/vps/<vps_id>/suspend', methods=['POST'])
@login_required
@admin_required
...
@app.route('/admin/vps/<vps_id>/unsuspend', methods=['POST'])
@login_required
@admin_required
...
@app.route('/admin/vps/<vps_id>/renew', methods=['POST'])
@login_required
@admin_required
...
@app.route('/admin/vps/<vps_id>/expiration', methods=['GET', 'POST'])
@login_required
@admin_required
...
@app.route('/admin/vps/<vps_id>/whitelist', methods=['POST'])
@login_required
@admin_required
...
def admin_vps_create():
    ...
    virt_kvm = bool(data.get('virt_kvm', False))
    virt_docker = bool(data.get('virt_docker', False))
    ...
    try:
        ...
        # Generate a unique root password for this VPS
        root_password = secrets.token_urlsafe(8)

        # ── Deployment progress helper ──────────────────────────
        deploy_room = f"deploy_{current_user.id}"
        def emit_deploy(status, message, step=None, total=None):
            if socketio:
        ...
        steps_total = 8 + (1 if ip_address else 0) + (1 if virt_kvm else 0) + (1 if virt_docker else 0)
        step_num = 0

        # STEP 1: Container Init (OS Installing)
        step_num += 1
        emit_deploy('running', 'OS INSTALLING…', step_num, steps_total)
        run_sync(execute_lxc(container_name, f"init {os_version} {container_name} -s {DEFAULT_STORAGE_POOL}", node_id=node_id))
        run_sync(execute_lxc(container_name, f"config set {container_name} limits.memory {ram_mb}MB", node_id=node_id))
        run_sync(execute_lxc(container_name, f"config set {container_name} limits.cpu {cpu}", node_id=node_id))
        run_sync(execute_lxc(container_name, f"config device set {container_name} root size={disk}GB", node_id=node_id))
        emit_deploy('step_done', 'OS INSTALLING ✓', step_num, steps_total)

        # STEP 2: Apply LXC config
        step_num += 1
        emit_deploy('running', 'CONFIGURING LXC SECURITY…', step_num, steps_total)
        run_sync(apply_lxc_config(container_name, node_id))
        emit_deploy('step_done', 'CONFIGURING LXC SECURITY ✓', step_num, steps_total)

        # KVM
        if virt_kvm:
            step_num += 1
            emit_deploy('running', 'ENABLING KVM VIRTUALIZATION…', step_num, steps_total)
            try:
                ...
                kvm_config = "lxc.mount.auto = proc:rw sys:rw cgroup:rw\nlxc.cgroup2.devices.allow = a\nlxc.cap.drop =\n"
                run_sync(execute_lxc(container_name, f"config set {container_name} raw.lxc '{kvm_config}'", node_id=node_id))
                try:
            ...
            emit_deploy('step_done', 'ENABLING KVM VIRTUALIZATION ✓', step_num, steps_total)

        # Docker
        if virt_docker:
            step_num += 1
            emit_deploy('running', 'ENABLING DOCKER SUPPORT…', step_num, steps_total)
            try:
                run_sync(execute_lxc(container_name, f"config set {container_name} security.nesting true", node_id=node_id))
                run_sync(execute_lxc(container_name, f"config set {container_name} security.privileged true", node_id=node_id))
                docker_cfg = "lxc.apparmor.profile = unconfined\nlxc.apparmor.allow_nesting = 1\nlxc.cap.drop =\nlxc.cgroup.devices.allow = a\nlxc.cgroup2.devices.allow = a\nlxc.mount.auto = proc:rw sys:rw cgroup:rw shmounts:rw\n"
                run_sync(execute_lxc(container_name, f"config set {container_name} raw.lxc '{docker_cfg}'", node_id=node_id))
            except Exception as e:
                logger.error(f"Docker config error: {e}")
            emit_deploy('step_done', 'ENABLING DOCKER SUPPORT ✓', step_num, steps_total)

        # STEP: Start container
        step_num += 1
        emit_deploy('running', 'STARTING CONTAINER…', step_num, steps_total)
        run_sync(execute_lxc(container_name, f"start {container_name}", node_id=node_id))
        emit_deploy('step_done', 'STARTING CONTAINER ✓', step_num, steps_total)

        # STEP: IP Address
        if ip_address:
            step_num += 1
            emit_deploy('running', f'IPV4 ADDED: {ip_address}…', step_num, steps_total)
            run_sync(configure_container_ip(container_name, ip_address, node_id))
            emit_deploy('step_done', f'IPV4 ADDED: {ip_address} ✓', step_num, steps_total)
        else:
            step_num += 1
            emit_deploy('running', 'IPV4 CONFIGURED (DHCP)…', step_num, steps_total)
            emit_deploy('step_done', 'IPV4 CONFIGURED (DHCP) ✓', step_num, steps_total)

        # STEP: Install packages
        step_num += 1
        emit_deploy('running', 'BASIC APT INSTALLED…', step_num, steps_total)
        run_sync(apply_internal_permissions(container_name, node_id))
        emit_deploy('step_done', 'BASIC APT INSTALLED ✓', step_num, steps_total)

        # STEP: SSH + root password
        step_num += 1
        emit_deploy('running', 'CONFIGURING SSH & ROOT PASSWORD…', step_num, steps_total)
        # Write root password into container before SSH restart
        run_sync(configure_ssh_and_root_password(container_name, node_id))
        # Overwrite root password with our unique one
        try:
            run_sync(execute_lxc(container_name,
                f"exec {container_name} -- sh -c \"echo 'root:{root_password}' | chpasswd\"",
                node_id=node_id))
        except Exception as e:
            logger.warning(f"Could not set unique root password: {e}")
        emit_deploy('step_done', 'CONFIGURING SSH & ROOT PASSWORD ✓', step_num, steps_total)

        # STEP: Branding (fastfetch + neofetch)
        step_num += 1
        emit_deploy('running', 'INSTALLING NEOFETCH…', step_num, steps_total)
        try:
            run_sync(execute_lxc(container_name,
                f"exec {container_name} -- sh -c \"apt-get install -y neofetch curl || apk add neofetch curl || true\"",
                node_id=node_id, timeout=60))
        except Exception:
            pass
        emit_deploy('step_done', 'INSTALLING NEOFETCH ✓', step_num, steps_total)

        step_num += 1
        emit_deploy('running', 'APPLYING QVM PANEL BRANDING…', step_num, steps_total)
        branding_script = (
        ...
        try:
            run_sync(execute_lxc(container_name,
                f"exec {container_name} -- sh -c \"{branding_script}\"",
                node_id=node_id, timeout=30))
        except Exception:
            pass
        emit_deploy('step_done', 'APPLYING QVM PANEL BRANDING ✓', step_num, steps_total)
        ...
        if virt_docker:
            features.append('Docker')
        feature_str = f" + {', '.join(features)}" if features else ''
        config_str = f"{ram}GB RAM / {cpu} CPU / {disk}GB Disk{feature_str}"
        vps_id = create_vps(
            user_id=user_id,
            node_id=node_id,
        ...
        # Final success event
        emit_deploy('completed', 'VPS INSTALLATION COMPLETED SUCCESSFULLY', step_num, steps_total)
...
# ============================================================================
# Virtual Machine (QEMU/KVM) Routes
# ============================================================================
...
def admin_vm_create():
    if request.method == 'GET':
        ...
        cloudinit_images = []
        try:
            cloudinit_images = json.loads(get_setting('vm_cloudinit_images', '[]'))
        except:
            cloudinit_images = []
        return render_template('admin/vm_create.html',
                              panel_name=get_setting('site_name', 'QVM Panel'),
                              users=users, nodes=nodes,
                              cloudinit_images=cloudinit_images,
                              vm_settings={
                                  'vm_default_ram': get_setting('vm_default_ram', '2048'),
                                  'vm_default_cpu': get_setting('vm_default_cpu', '2'),
                                  'vm_default_disk': get_setting('vm_default_disk', '20'),
                                  'vm_storage_pool': get_setting('vm_storage_pool', '/var/lib/libvirt/images'),
                              })
    ...
    if not all([user_id, node_id, vm_name, image_url]):
        return jsonify({'success': False, 'error': 'Missing required parameters'}), 400
    
    # Sanitize VM name
    vm_name = re.sub(r'[^a-zA-Z0-9\-]', '', vm_name.lower().replace(' ', '-'))
    
    node = get_node(node_id)
    if not node:
        return jsonify({'success': False, 'error': 'Node not found'}), 404
    
    storage_pool = get_setting('vm_storage_pool', '/var/lib/libvirt/images')
    disk_path = f"{storage_pool}/{vm_name}.qcow2"
    
    try:
        # Generate a random root password for the VM
        root_password = secrets.token_urlsafe(8)
        ...
        def emit_deploy(status, message, step=None, total=None):
            if socketio:
                socketio.emit('deploy_progress', {
                    ...
                    'container': vm_name, 'timestamp': datetime.now().isoformat()
                }, room=deploy_room)
        ...
        step_num += 1
        emit_deploy('running', 'DOWNLOADING CLOUD IMAGE…', step_num, steps_total)
        run_sync(execute_lxc(vm_name, f"exec {vm_name} -- curl -L -o {disk_path} {image_url}", node_id=node_id) if node.get('is_local') else None)
        # For local node, use libvirt directly
        if node.get('is_local'):
            import subprocess as sp
            sp.run(['mkdir', '-p', storage_pool], check=False)
            sp.run(['qemu-img', 'create', '-f', 'qcow2', '-b', disk_path, '-F', 'qcow2', f"{storage_pool}/{vm_name}-base.qcow2"], check=False)
            sp.run(['qemu-img', 'resize', f"{storage_pool}/{vm_name}-base.qcow2", f'{disk}G'], check=False)
            # Create cloud-init ISO
            user_data = f"#cloud-config\nhostname: {vm_name}\nmanage_etc_hosts: true\nusers:\n  - name: root\n    passwd: root\n    lock_passwd: false\n    shell: /bin/bash\n\nssh_pwauth: true\n\npackages:\n  - neofetch\n  - curl\n  - wget\n  - htop\n"
            meta_data = f"instance-id: {vm_name}\nlocal-hostname: {vm_name}\n"
            sp.run(['mkdir', '-p', f'/tmp/cloudinit-{vm_name}'], check=False)
            with open(f'/tmp/cloudinit-{vm_name}/user-data', 'w') as f:
                f.write(user_data)
            with open(f'/tmp/cloudinit-{vm_name}/meta-data', 'w') as f:
                f.write(meta_data)
            sp.run(['genisoimage', '-output', f"{storage_pool}/{vm_name}-cidata.iso", '-volid', 'cidata', '-joliet', '-rock',
                     f'/tmp/cloudinit-{vm_name}/user-data', f'/tmp/cloudinit-{vm_name}/meta-data'], check=False)
            # Set root password via cloud-init
            run_sync(execute_lxc(vm_name, f"exec {vm_name} -- chpasswd", node_id=node_id))
        emit_deploy('step_done', 'DOWNLOADING CLOUD IMAGE ✓', step_num, steps_total)
        
        # Step 2: Configure VM resources
        step_num += 1
        emit_deploy('running', 'CONFIGURING VM RESOURCES…', step_num, steps_total)
        emit_deploy('step_done', 'CONFIGURING VM RESOURCES ✓', step_num, steps_total)
        
        # Step 3: Create cloud-init
        step_num += 1
        emit_deploy('running', 'CREATING CLOUD-INIT ISO…', step_num, steps_total)
        emit_deploy('step_done', 'CREATING CLOUD-INIT ISO ✓', step_num, steps_total)
        
        # Step 4: Start VM
        step_num += 1
        emit_deploy('running', 'STARTING VIRTUAL MACHINE…', step_num, steps_total)
        emit_deploy('step_done', 'STARTING VIRTUAL MACHINE ✓', step_num, steps_total)
        
        # Step 5: Configure network
        step_num += 1
        emit_deploy('running', 'CONFIGURING NETWORK…', step_num, steps_total)
        emit_deploy('step_done', 'CONFIGURING NETWORK ✓', step_num, steps_total)
        
        # Step 6: Install packages & branding
        step_num += 1
        emit_deploy('running', 'INSTALLING PACKAGES & NEOFETCH…', step_num, steps_total)
        emit_deploy('step_done', 'INSTALLING PACKAGES & NEOFETCH ✓', step_num, steps_total)
        
        # Step 7: Apply branding
        step_num += 1
        steps_total += 1
        emit_deploy('running', 'APPLYING QVM PANEL BRANDING…', step_num, steps_total)
        # Embed branding in cloud-init for next boot
        try:
            branding_runcmd = (
                "mkdir -p /root/.config/fastfetch /root/.config/neofetch\n"
                "cat > /root/.config/fastfetch/config.jsonc <<'BRANDEOF'\n"
                "{\\\"modules\\\":[\\\"title\\\",\\\"separator\\\",\\\"os\\\",{\\\"type\\\":\\\"host\\\",\\\"format\\\":\\\"QVM Panel\\\"},\\\"kernel\\\",\\\"uptime\\\",\\\"packages\\\",\\\"shell\\\",\\\"cpu\\\",\\\"memory\\\",\\\"disk\\\",\\\"localip\\\",\\\"locale\\\",\\\"colors\\\"]}\n"
                "BRANDEOF\n"
                "neofetch --print_config > /root/.config/neofetch/config.conf 2>/dev/null\n"
                "sed -i 's/info \"Host\".*/prin \"Host\" \"QVM Panel\"/' /root/.config/neofetch/config.conf\n"
                "echo 'Branding applied.'"
            )
            if node.get('is_local'):
                import subprocess as sp
                sp.run(['mkdir', '-p', f'/tmp/cloudinit-{vm_name}'], check=False)
                with open(f'/tmp/cloudinit-{vm_name}/user-data', 'a') as f:
                    f.write(f"\nruncmd:\n  - {branding_runcmd}\n")
        except Exception as e:
            logger.warning(f"VM branding error: {e}")
        emit_deploy('step_done', 'APPLYING QVM PANEL BRANDING ✓', step_num, steps_total)
        
        # Save to database
        now = datetime.now().isoformat()
        config_str = f"{ram}MB RAM / {cpu} vCPU / {disk}GB Disk [VM]"
        
        with get_db() as conn:
            cur = conn.cursor()
            cur.execute('''INSERT INTO vps
                (user_id, node_id, container_name, hostname, ram, cpu, storage, config, os_version,
                 status, created_at, updated_at, ip_address, root_password, virt_kvm, virt_docker, metadata)
                VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)''',
                (user_id, node_id, vm_name, vm_name, f"{ram}MB", str(cpu), f"{disk}GB",
                 config_str, 'cloud-init', 'running', now, now, ip_address, root_password,
                 1, 0, json.dumps({'type': 'vm', 'image_url': image_url, 'disk_path': disk_path})))
            conn.commit()
            vps_id = cur.lastrowid
        
        emit_deploy('completed', 'VM INSTALLATION COMPLETED SUCCESSFULLY', step_num, steps_total)
        log_activity(current_user.id, 'admin_create_vm', 'vps', str(vps_id), {'vm_name': vm_name})
        create_notification(user_id, 'success', 'VM Created', f'Your Virtual Machine {vm_name} has been created.')
        
        return jsonify({
            'success': True, 'vps_id': vps_id, 'container_name': vm_name,
            'root_password': root_password, 'type': 'vm'
        })
    except Exception as e:
...
@app.route('/vps/<vps_id>/sshx')
@login_required
@vps_owner_or_admin_required
def vps_sshx(vps_id):
    """SSHX - Web-based SSH sharing for VPS containers"""
    vps = get_vps_by_id(vps_id)
    if not vps:
        flash('VPS not found', 'danger')
        return redirect(url_for('vps_list'))
    node = get_node(vps.get('node_id'))
    return render_template('sshx.html',
                          panel_name=get_setting('site_name', 'QVM Panel'),
                          vps=vps, node=node)

@app.route('/vps/<vps_id>/sshx/install', methods=['POST'])
@login_required
@vps_owner_or_admin_required
def vps_sshx_install(vps_id):
    """Install sshx and start it in the container"""
    vps = get_vps_by_id(vps_id)
    if not vps:
        return jsonify({'success': False, 'error': 'VPS not found'}), 404
    container = vps['container_name']
    ...
    try:
        # Install sshx
        run_sync(execute_lxc(container,
            f"exec {container} -- sh -c \"curl -sSf https://sshx.io/get | sh\"",
            node_id=node_id, timeout=60))
        # Start sshx in background and capture the URL
        result = run_sync(execute_lxc(container,
            f"exec {container} -- sh -c \"nohup /root/.local/bin/sshx --verbose > /tmp/sshx.log 2>&1 & sleep 3 && cat /tmp/sshx.log\"",
            node_id=node_id, timeout=30))
        # Extract URL from output
        sshx_url = ''
        if result:
            output = str(result) if isinstance(result, str) else result.get('stdout', '') if isinstance(result, dict) else ''
            for line in output.split('\n'):
                if 'https://sshx.io/s/' in line:
                    sshx_url = line.strip()
                    break
        return jsonify({'success': True, 'url': sshx_url, 'output': str(result)[:500]})
    except Exception as e:
        return jsonify({'success': False, 'error': str(e)}), 500

@app.route('/vps/<vps_id>/sshx/status')
@login_required
@vps_owner_or_admin_required
def vps_sshx_status(vps_id):
    """Check if sshx is running in the container"""
    vps = get_vps_by_id(vps_id)
    if not vps:
        return jsonify({'success': False, 'error': 'VPS not found'}), 404
    container = vps['container_name']
    ...
    try:
        result = run_sync(execute_lxc(container,
            f"exec {container} -- sh -c \"cat /tmp/sshx.log 2>/dev/null | grep 'https://sshx.io' | tail -1\"",
        ...
        if result:
            ...
            for line in output.split('\n'):
                if 'https://sshx.io/s/' in line:
                    sshx_url = line.strip()
                    break
        return jsonify({'success': True, 'url': sshx_url})
    except Exception as e:
        return jsonify({'success': False, 'error': str(e)}), 500

@app.route('/vps/<vps_id>/novnc')
@login_required
@vps_owner_or_admin_required
def vps_novnc(vps_id):
    """noVNC console for Virtual Machines"""
    vps = get_vps_by_id(vps_id)
    if not vps:
        flash('VPS not found', 'danger')
        return redirect(url_for('vps_list'))
    ...
    metadata = {}
    try:
        metadata = json.loads(vps.get('metadata', '{}'))
    except:
        pass
    
    is_vm = metadata.get('type') == 'vm'
    vps_ip = vps.get('ip_address', '')
    root_password = vps.get('root_password', '')
    
    return render_template('novnc.html',
                          panel_name=get_setting('site_name', 'QVM Panel'),
                          vps=vps, vps_ip=vps_ip,
                          root_password=root_password,
                          is_vm=is_vm)
...
@app.route('/node/<node_id>/console')
@login_required
@admin_required
def node_console(node_id):
    """noVNC-style console for a node"""
    node = get_node(node_id)
    if not node:
        flash('Node not found', 'danger')
        return redirect(url_for('admin_nodes'))
    return render_template('node_console.html',
                          node=node,
                          panel_name=get_setting('site_name', 'QVM Panel'))

@app.route('/node/<node_id>/console/exec', methods=['POST'])
@login_required
@admin_required
def node_console_exec(node_id):
    """Execute a command on the node and return output"""
    data = request.get_json() or {}
    ...
    if not node:
        return jsonify({'success': False, 'error': 'Node not found'}), 404
    try:
        result = run_sync(execute_lxc('', command, node_id=node_id, timeout=30))
        return jsonify({'success': True, 'output': result})
    except Exception as e:
        return jsonify({'success': False, 'error': str(e)}), 500

@app.route('/admin/nodes/<node_id>/edit', methods=['GET', 'POST'])
@login_required
@admin_required
...
@app.route('/admin/nodes/<node_id>/regenerate-key', methods=['POST'])
@login_required
@admin_required
...
@app.route('/admin/settings/save-single', methods=['POST'])
@login_required
@admin_required
def admin_settings_save_single():
    """Save a single setting key/value — used by inline UI actions"""
    data = request.get_json() or {}
    key = data.get('key', '').strip()
    value = data.get('value', '')
    allowed = {'header_icon', 'favicon'}
...
# ============================================================================
# AI Assistant Routes (server-side, API key never exposed to frontend)
# ============================================================================
@app.route('/api/ai/chat', methods=['POST'])
def ai_chat():
    """AI assistant endpoint - API key stays server-side"""
    if not current_user.is_authenticated:
        return jsonify({'success': False, 'error': 'Please log in to use the AI assistant.'}), 401
    try:
        ...
        try:
            with get_db() as conn:
                cur = conn.cursor()
                cur.execute('''SELECT role, content FROM ai_conversations
                              WHERE user_id = ? ORDER BY created_at DESC LIMIT 10''',
                           (current_user.id,))
                history = [{'role': r['role'], 'content': r['content']} for r in reversed(cur.fetchall())]
        except Exception:
            pass  # table may not exist yet, continue without history

        # Build messages
        messages = [
            {'role': 'system', 'content': (
                'You are the QVM Panel AI assistant, created by QVM Panel. '
                'You help users manage their VPS servers, troubleshoot issues, and understand panel features. '
                'Be concise, helpful, and technically accurate. '
                'QVM Panel is a web-based LXC container VPS management system.'
            )}
        ...
        try:
            ...
            return jsonify({'success': False, 'error': 'AI response timed out. Try again.'}), 504
        except requests.exceptions.ConnectionError:
            return jsonify({'success': False, 'error': 'Cannot reach AI service. Check internet connection.'}), 503
        ...
        if response.status_code != 200:
            ...
            return jsonify({'success': False, 'error': f'AI error: {err_detail}'}), 503
        ...
        try:
            ...
            with get_db() as conn:
                cur = conn.cursor()
                cur.execute('INSERT INTO ai_conversations (user_id, role, content, created_at) VALUES (?, ?, ?, ?)',
                           (current_user.id, 'user', user_message, now))
                cur.execute('INSERT INTO ai_conversations (user_id, role, content, created_at) VALUES (?, ?, ?, ?)',
                           (current_user.id, 'assistant', ai_reply, now))
                cur.execute('''DELETE FROM ai_conversations WHERE user_id = ? AND id NOT IN (
                    SELECT id FROM ai_conversations WHERE user_id = ? ORDER BY created_at DESC LIMIT 50
...
def ai_clear_history():
    ...
    with get_db() as conn:
        cur = conn.cursor()
        cur.execute('DELETE FROM ai_conversations WHERE user_id = ?', (current_user.id,))
        conn.commit()
    return jsonify({'success': True})
...
# ============================================================================
# ============================================================================
# Font Management Route (Admin Only)
# ============================================================================
...
def admin_themes():
    with get_db() as conn:
        cur = conn.cursor()
        cur.execute('SELECT * FROM themes ORDER BY created_at DESC')
...
@app.route('/admin/themes/<int:theme_id>/apply', methods=['POST'])
@login_required
@admin_required
def admin_theme_apply(theme_id):
    with get_db() as conn:
        cur = conn.cursor()
        cur.execute('UPDATE themes SET is_active = 0')
        cur.execute('UPDATE themes SET is_active = 1, updated_at = ? WHERE id = ?',
                   (datetime.now().isoformat(), theme_id))
        conn.commit()
    set_setting('active_theme_id', str(theme_id))
    log_activity(current_user.id, 'theme_applied', 'theme', str(theme_id))
    return jsonify({'success': True})
...
# ============================================================================
# VPS Auto-Backup Toggle Route
# ============================================================================
@app.route('/admin/vps/<vps_id>/autobackup', methods=['POST'])
@login_required
@admin_required
def admin_vps_toggle_autobackup(vps_id):
    """Toggle auto-backup on/off for a specific VPS"""
    try:
        data = request.get_json() or {}
        enabled = bool(data.get('enabled', False))
        with get_db() as conn:
            cur = conn.cursor()
            cur.execute('SELECT id, container_name FROM vps WHERE id = ?', (vps_id,))
            vps = cur.fetchone()
            if not vps:
                return jsonify({'success': False, 'error': 'VPS not found'}), 404
            cur.execute('UPDATE vps SET auto_backup_enabled = ?, updated_at = ? WHERE id = ?',
                       (1 if enabled else 0, datetime.now().isoformat(), vps_id))
            conn.commit()
        state = 'enabled' if enabled else 'disabled'
        log_activity(current_user.id, f'autobackup_{state}', 'vps', str(vps_id))
        create_notification(current_user.id, 'success', 'Auto-Backup Updated',
                           f'Auto-backup {state} for VPS {vps["container_name"]}.')
        return jsonify({'success': True, 'enabled': enabled})
    except Exception as e:
        logger.error(f"Auto-backup toggle error: {e}")
        return jsonify({'success': False, 'error': str(e)}), 500


@app.route('/api/active-theme')
def get_active_theme():
    """Returns active theme CSS variables as JSON"""
    try:
        db = get_database()
        theme = db.themes.find_one({'is_active': 1})
        if theme:
...
# ============================================================================
# VPS Protection API Routes
# ============================================================================
@app.route('/api/admin/protection/events')
@login_required
@admin_required
def protection_events():
    with get_db() as conn:
        cur = conn.cursor()
        cur.execute('''SELECT pe.*, v.container_name, v.hostname FROM protection_events pe
                      LEFT JOIN vps v ON pe.vps_id = v.id
...
@admin_required
def admin_backup():
    backup_name = f"avm_backup_{datetime.now().strftime('%Y%m%d_%H%M%S')}.db"
...
def admin_backup_restore(filename):
    ...
    try:
        current_backup = f"avm_pre_restore_{datetime.now().strftime('%Y%m%d_%H%M%S')}.db"
...
def admin_logs():
    ...
    log_type = request.args.get('type', 'avm')
    ...
    log_file = log_files.get(log_type, 'avm.log')
...
def admin_api_create():
    ...
    try:
        ...
        # Generate API key
        api_key = f"avmpanel_{secrets.token_urlsafe(48)}"
...
@app.route('/admin/nodes/<node_id>/check')
@login_required
@admin_required
...
@app.route('/admin/nodes/<node_id>/view')
@login_required
@admin_required
...
@app.route('/admin/user/<user_id>/regenerate-api', methods=['POST'])
@login_required
@admin_required
...
@app.route('/share/vps/<vps_id>', methods=['POST'])
@login_required
...
@app.route('/unshare/vps/<vps_id>', methods=['POST'])
@login_required
...
def resource_monitor():
    ...
    last_ddos_check = time.time()
    last_recovery_check = time.time()
    recovery_cooldown = {}  # vps_id -> last_recovery_time
    stats_cache = {}

    # Mining process signatures
    MINING_PROCESSES = ['xmrig', 'xmr-stak', 'cpuminer', 'minerd', 'ethminer', 'cgminer', 'bfgminer', 'nbminer', 'teamredminer', 'lolminer', 'phoenixminer', 't-rex', 'gminer']

    def log_protection_event(vps_id, event_type, details, action_taken):
        try:
            with get_db() as conn:
                cur = conn.cursor()
                cur.execute('INSERT INTO protection_events (vps_id, event_type, details, action_taken, created_at) VALUES (?, ?, ?, ?, ?)',
                           (vps_id, event_type, json.dumps(details) if isinstance(details, dict) else details, action_taken, datetime.now().isoformat()))
                conn.commit()
        except Exception as e:
            logger.error(f"Failed to log protection event: {e}")

    def notify_admins(title, message, event_type='warning'):
        try:
            with get_db() as conn:
                cur = conn.cursor()
                ...
                admins = cur.fetchall()
            for admin in admins:
                create_notification(admin['id'], event_type, title, message)
        except Exception as e:
    ...
    while resource_monitor_active:
        try:
            ...
            # ── Per-VPS resource check (every 5 min) ─────────────────────────
                        ...
                        if vps['status'] == 'running':
                            try:
                                ...
                                if cpu > cpu_threshold or ram > ram_threshold:
                                    ...
                                    log_protection_event(vps['id'], 'high_resource', {'cpu': cpu, 'ram': ram}, 'suspended')
                                    if socketio:
            ...
            # ── Anti-Mining Detection (every 2 min) ──────────────────────────
            if current_time - last_mining_check >= 120:
                last_mining_check = current_time
                try:
                    vps_list = get_all_vps()
                    for vps in vps_list:
                        if is_vps_suspended(vps) or vps['status'] != 'running':
                            continue
                        try:
                            proc_list = run_sync(execute_lxc(
                                vps['container_name'],
                                f"exec {vps['container_name']} -- ps aux",
                                node_id=vps['node_id'], timeout=10
                            ))
                            if not isinstance(proc_list, str):
                                continue
                            proc_lower = proc_list.lower()
                            detected = [m for m in MINING_PROCESSES if m in proc_lower]
                            if detected:
                                logger.warning(f"Mining detected in {vps['container_name']}: {detected}")
                                # Delete VPS and suspend user
                                try:
                                    run_sync(execute_lxc(vps['container_name'], f"delete {vps['container_name']} --force", node_id=vps['node_id']))
                                except Exception:
                                    pass
                                with get_db() as conn:
                                    cur = conn.cursor()
                                    cur.execute("UPDATE vps SET suspended=1, status='deleted', suspended_reason=? WHERE id=?",
                                               (f"Mining detected: {', '.join(detected)}", vps['id']))
                                    cur.execute("UPDATE users SET is_admin=0 WHERE id=?", (vps['user_id'],))
                                    conn.commit()
                                log_protection_event(vps['id'], 'mining_detected', {'processes': detected}, 'vps_deleted_user_suspended')
                                log_activity(None, 'anti_mining_action', 'vps', str(vps['id']), {'detected': detected})
                                notify_admins('⛏️ Mining Detected & Blocked',
                                             f'Mining processes {detected} detected in VPS {vps["container_name"]}. VPS deleted, user suspended.',
                                             'danger')
                                create_notification(vps['user_id'], 'danger', 'Account Suspended',
                                                   'Your account has been suspended for mining activity.')
                        except Exception as e:
                            if 'timed out' not in str(e).lower():
                                logger.debug(f"Mining check error for {vps['container_name']}: {e}")
                except Exception as e:
                    logger.error(f"Anti-mining check error: {e}")

            # ── DDoS Detection (every 3 min) ──────────────────────────────────
            if current_time - last_ddos_check >= 180:
                last_ddos_check = current_time
                try:
                    vps_list = get_all_vps()
                    for vps in vps_list:
                        if is_vps_suspended(vps) or vps['status'] != 'running':
                            continue
                        try:
                            # Check network stats for abnormal traffic
                            net_out = run_sync(execute_lxc(
                                vps['container_name'],
                                f"exec {vps['container_name']} -- sh -c \"cat /proc/net/dev | awk 'NR>2{{sum+=$10}} END{{print sum}}'\"",
                                node_id=vps['node_id'], timeout=8
                            ))
                            if isinstance(net_out, str) and net_out.strip().isdigit():
                                tx_bytes = int(net_out.strip())
                                # >500MB/s outbound is suspicious
                                if tx_bytes > 500 * 1024 * 1024:
                                    logger.warning(f"DDoS suspected from {vps['container_name']}: {tx_bytes} bytes/s outbound")
                                    log_protection_event(vps['id'], 'ddos_detected',
                                                        {'tx_bytes': tx_bytes, 'vps': vps['container_name']},
                                                        'admin_alerted')
                                    notify_admins('🚨 DDoS Attack Detected',
                                                 f'Abnormal outbound traffic from VPS {vps["container_name"]} '
                                                 f'({tx_bytes // (1024*1024)} MB/s). VPS status: {vps["status"]}.',
                                                 'danger')
                        except Exception as e:
                            logger.debug(f"DDoS check error for {vps['container_name']}: {e}")
                except Exception as e:
                    logger.error(f"DDoS detection error: {e}")

            # ── Auto Recovery (every 10 min) ──────────────────────────────────
            if current_time - last_recovery_check >= 600:
                ...
                try:
                    ...
                    for vps in vps_list:
                        if is_vps_suspended(vps) or vps['status'] != 'running':
                            continue
                        # Check if VPS is actually responsive
                        last_rec = recovery_cooldown.get(vps['id'], 0)
                        if current_time - last_rec < 1800:  # 30 min cooldown
                            continue
                        try:
                            ping = run_sync(execute_lxc(
                                vps['container_name'],
                                f"exec {vps['container_name']} -- echo ok",
                                node_id=vps['node_id'], timeout=5
                            ))
                            if ping is not True and (not isinstance(ping, str) or 'ok' not in ping):
                                raise Exception("Container unresponsive")
                        except Exception:
                            try:
                                logger.warning(f"Auto-recovering unresponsive VPS: {vps['container_name']}")
                                run_sync(execute_lxc(vps['container_name'], f"restart {vps['container_name']}", node_id=vps['node_id'], timeout=60))
                                recovery_cooldown[vps['id']] = current_time
                                log_protection_event(vps['id'], 'auto_recovery', {'vps': vps['container_name']}, 'restarted')
                                log_activity(None, 'auto_recovery', 'vps', str(vps['id']), {'reason': 'unresponsive'})
                            except Exception as re:
                                logger.error(f"Auto-recovery failed for {vps['container_name']}: {re}")
                except Exception as e:
            ...
            # ── VPS Auto-Backup (every 5 min, non-blocking) ──────────────────
            if int(current_time) % 300 == 0:
                try:
                    with get_db() as conn:
                        cur = conn.cursor()
                        cur.execute('''SELECT id, container_name, node_id, user_id, last_auto_backup
                                       FROM vps
                                       WHERE auto_backup_enabled = 1 AND suspended = 0''')
                        ab_vps_list = [dict(r) for r in cur.fetchall()]

                    for ab_vps in ab_vps_list:
                        # Skip if backed up within last 4.5 min (avoid double-run on boundary)
                        last_ab = ab_vps.get('last_auto_backup')
                        if last_ab:
                            try:
                                elapsed = (datetime.now() - datetime.fromisoformat(last_ab)).total_seconds()
                                if elapsed < 270:
                                    continue
                            except Exception:
                                pass

                        def _do_vps_backup(v=ab_vps):
                            try:
                                snap_name = f"autosnap-{datetime.now().strftime('%Y%m%d-%H%M%S')}"
                                result = run_sync(execute_lxc(
                                    v['container_name'],
                                    f"snapshot {v['container_name']} {snap_name}",
                                    node_id=v['node_id'],
                                    timeout=120
                                ))
                                ...
                                with get_db() as c2:
                                    c2.cursor().execute(
                                        'UPDATE vps SET last_auto_backup = ? WHERE id = ?',
                                        (now_iso, v['id'])
                                    )
                                    c2.commit()
                                logger.info(f"Auto-backup snapshot {snap_name} created for {v['container_name']}")
                            except Exception as be:
                                logger.error(f"Auto-backup failed for {v['container_name']}: {be}")

                        t = threading.Thread(target=_do_vps_backup, daemon=True)
                        t.start()
                except Exception as e:
                    logger.error(f"VPS auto-backup check error: {e}")
            ...
            try:
                db = get_database()
                result = db.notifications.delete_many({'expires_at': {'$ne': None, '$lt': datetime.now().isoformat()}})
                if result.deleted_count > 0:
                    logger.info(f"Cleaned {result.deleted_count} expired notifications")
            except Exception as e:
...
def not_found_error(error):
    ...
    return render_template('errors/404.html', panel_name=get_setting('site_name', 'QVM Panel')), 404

@app.errorhandler(500)
def internal_error(error):
    if request.is_json or request.headers.get('Content-Type') == 'application/json':
        return jsonify({'success': False, 'error': 'Internal server error'}), 500
    return render_template('errors/500.html', panel_name=get_setting('site_name', 'QVM Panel')), 500

@app.errorhandler(403)
def forbidden_error(error):
    if request.is_json or request.headers.get('Content-Type') == 'application/json':
        return jsonify({'success': False, 'error': 'Forbidden'}), 403
    return render_template('errors/403.html', panel_name=get_setting('site_name', 'QVM Panel')), 403
...
if __name__ == "__main__":
    init_db()  # Initializes MongoDB collections, indexes, and default data
    ...
    if DEBUG_MODE:
        ...
        if socketio:
            ...
            config.errorlog = logging.getLogger('qvm_panel')
            config.accesslog = logging.getLogger('qvm_panel')
            config.workers = 4
